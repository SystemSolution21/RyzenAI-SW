...
def get_npu_info():
    ...
    if "PCI\\VEN_1022&DEV_17F0&REV_11" in stdout.decode(
        ...
        apu_type = "STX"
    if "PCI\\VEN_1022&DEV_17F0&REV_20" in stdout.decode(
        encoding="utf-8", errors="ignore"
    ):
        apu_type = "KRK"
    return apu_type
...
def get_xclbin(npu_device):
    ...
    if npu_device == "STX" or npu_device == "KRK":
        xclbin_file = "{}\\voe-4.0-win_amd64\\xclbins\\strix\\AMD_AIE2P_4x4_Overlay.xclbin".format(
            os.environ["RYZEN_AI_INSTALLATION_PATH"]
        )
    if npu_device == "PHX/HPT":
        xclbin_file = "{}\\voe-4.0-win_amd64\\xclbins\\phoenix\\4x4.xclbin".format(
            os.environ["RYZEN_AI_INSTALLATION_PATH"]
        )
    return xclbin_file
